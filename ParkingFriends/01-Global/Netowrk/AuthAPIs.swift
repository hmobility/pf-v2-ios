...
struct AuthAPI:BaseAPI {
    ...
    // 휴대폰 OTP 전송
    static func otp(httpMethod:HttpMethod = .post, auth:APIAuthType = .serviceKey) -> RestURL  {
    ...
    // 아이디/비밀번호 찾기 이메일 전송
    static func email(type:AuthEmailType, httpMethod:HttpMethod = .post, auth:APIAuthType = .serviceKey) -> RestURL  {
